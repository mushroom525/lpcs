...
<div class="tdare">
    <table width="100%" cellspacing="0" class="dataTable" style="font-size:12px">
        ...
        <?php if(!empty($goods)): if(is_array($goods)): foreach($goods as $key=>$item): ?><tr class="tatr2">
            ...
            <td><span><img src="/wxmall/<?php echo ($item["goods_img"]); ?>" width="50px" height="50px"/></span></td>
            <td><?php echo ($item["goods_name"]); ?></td>
            <td class="align_center"><td><?php echo (htmlentities($item["cate_name"])); ?></td></td>
            <td class="table-center"><?php if($item['if_show'] == 1): ?><img src="/wxmall/Public/images/positive_enabled.gif" ectype="inline_edit" fieldname="if_show" /><?php else: ?><img src="/wxmall/Public/images/positive_disabled.gif" ectype="inline_edit" fieldname="if_show"><?php endif; ?></td>
        ...
        layer.confirm('您确定要删除该商品吗？', {
            ...
            $.post("<?php echo U('del');?>",{id:id},function(data){
                if(data.status==1){
                    ...
                    function location(){window.location.href = "http://localhost/wxmall/Admin/goods/index.html";};
                    setTimeout(function(){
                        location();
                    },1500);
                }else{
